...
class AN_ajax_admin extends CI_Controller
{
	...
	function edit_nama(){
		...
		$query=$this->db->query("UPDATE komentar SET nama='$nama' WHERE id='$id'");
		if($query){
			echo "ok";
		}
	}
	function rubah_email(){
		$id=changequote($this->input->post("id"));
		$email=trim(changequote($this->input->post("email")));
		$query=$this->db->query("UPDATE komentar SET email='$email' WHERE id='$id'");
		if($query){
			echo "ok";
		}
	}
	function edit_pesan(){
		$id=changequote($this->input->post("id"));
		$pesan=trim(changequote($this->input->post("pesan")));
		$query=$this->db->query("UPDATE komentar SET pesan='$pesan' WHERE id='$id'");
		if($query){
			echo "ok";
		}
	}
	...
	function delete_komentar(){
		$id=changequote($this->input->post("id"));
		$query=$this->db->query("DELETE FROM komentar WHERE id='$id'");
		if($query){
			echo "ok";
		}
	}
...
//============================ akhir artikel & awal diskusi ===================
		...
		function foto_gallery_diskusi(){

		$config=array(
			"upload_path"=>FCPATH."an-component/media/upload-gambar-diskusi/",
			"allowed_types"=>"gif|jpg|jpeg|png"
			);
		$this->load->library('upload', $config);
		 if ( ! $this->upload->do_upload())
                ...
                else
                {
                	...
                	if($id==0){
                	$query=$this->db->query("INSERT INTO foto_diskusi_temp (nama_foto,token_foto,sesi_form,id_user) VALUES ('$nama','$token_foto','$sesi','$id')");
                	} else {
                	$query=$this->db->query("INSERT INTO foto_diskusi (id_diskusi,nama_foto,token_foto) VALUES ('$id','$nama','$token_foto')");
               		 }
                	...
                	$config2=array(
                		"source_image"=>$this->upload->data('full_path'),
                		"maintain_ratio"=>TRUE,
                		"new_image"=>FCPATH."an-component/media/upload-gambar-diskusi-thumbs/",
                		//"create_thumb"=>TRUE,
                		"width"=>$width_thumb
                		);
                	...
                	if ( ! $this->image_lib->resize())
							{
								...
								$search=$this->db->query("SELECT id_foto FROM foto_diskusi WHERE token_foto='$token_foto' ");
								$dat=$search->row();
								$dat_id=$dat->id_foto;
								echo '{"gambar":"'.$nama.'","id":"'.$dat_id.'"}';
								}
	...
	function set_featured_image_diskusi(){
		$id=changequote($this->input->post("id"));
		$diskusi_id=changequote($this->input->post("diskusi_id"));
		$reset=$this->db->query("UPDATE foto_diskusi SET featured='N' WHERE featured='Y' AND id_diskusi='$diskusi_id' ");
		$update=$this->db->query("UPDATE foto_diskusi SET featured='Y' WHERE id_foto='$id'");
	     echo "ok";
	}


	function delete_foto_diskusi_temp(){
		$token_foto=changequote($this->input->post("token"));
		$query=$this->db->query("SELECT * FROM foto_diskusi_temp WHERE token_foto='$token_foto' ");
		if($query->num_rows()>0){
			$data=$query->row();
			$file=FCPATH."an-component/media/upload-gambar-diskusi/".$data->nama_foto;
			$thumbnail=FCPATH."an-component/media/upload-gambar-diskusi-thumbs/".$data->nama_foto;
			$query2=$this->db->query("DELETE FROM foto_diskusi_temp WHERE token_foto='$token_foto' ");
			unlink($file);
			unlink($thumbnail);
		}
	}

	function submit_diskusi($i=0){
		$judul=changequote($this->input->post('judul'));
		$sesi=changequote($this->input->post('sesi'));
		$id=changequote($this->input->post('id'));
		$seo=changequote($this->input->post('seo'));
		...
		$og_title=changequote($this->input->post('og_title'));
		$og_image=changequote($this->input->post('og_image'));
		$sesi_diskusi=changequote($this->input->post('sesi_diskusi'));
		$aStatus=changequote($this->input->post('aStatus'));
		...
		if($i==0){
			...
			//cek apakah sudah pernah ada sbg draft
			$search=$this->db->query("SELECT * FROM diskusi WHERE diskusi_sesi_id='$sesi' ");
			if($search->num_rows()>0){
				$update="UPDATE diskusi SET 
									diskusi_judul='$judul',
									diskusi_isi='$isi',
									diskusi_kategori='$kategori',
									diskusi_tags='$tag',
									diskusi_sbg_headline='$headline',
									diskusi_tgl_posting='$tgl_posting',
									diskusi_id_user='$id_user_input',
									diskusi_editable='$editable',
									diskusi_seo_url='$seo',
									diskusi_meta_description='$meta_description',
									diskusi_meta_author='$meta_author',
									diskusi_meta_keyword='$meta_keyword',
									diskusi_og_image='$og_image',
									diskusi_og_title='$og_title',
									diskusi_og_description='$og_description',
									diskusi_status='$stats',
									diskusi_was_published='$was_published'
									WHERE
									diskusi_sesi_id='$sesi'
							";
					$query=$this->db->query($update);
			} else {
				//belum ada di draft
				
				$ready= "INSERT INTO diskusi (
									diskusi_judul,
									diskusi_isi,
									diskusi_kategori,
									diskusi_tags,
									diskusi_sbg_headline,
									diskusi_tgl_posting,
									diskusi_id_user,
									diskusi_editable,
									diskusi_seo_url,
				...
				//memindahkan gambar ke diskusi


			}

			$tok=($this->input->post('returnDraft')=='true')?"diskusi_sesi_id='$sesi'":"diskusi_tgl_posting='$tanggal_posting'";	
				$sinc_diskusi=$this->db->query("SELECT diskusi_id FROM diskusi WHERE $tok");
				if($sinc_diskusi->num_rows()>0){
					$_id_diskusi=$sinc_diskusi->row();
					$_id_diskusi=$_id_diskusi->diskusi_id;


					$foto=$this->db->query("SELECT * FROM foto_diskusi_temp WHERE sesi_form='$sesi'");
					if($foto->num_rows()>0){
						foreach($foto->result_array() as $data){
							$this->db->query("INSERT INTO foto_diskusi (id_diskusi,nama_foto) VALUES ('$_id_diskusi','$data[nama_foto]') ");
							//delete row
							$this->db->query("DELETE FROM foto_diskusi_temp WHERE id_foto='$data[id_foto]'");
							
						}
					}
				}

				echo ($this->input->post('returnDraft')=='true')?"draftSaved":$_id_diskusi; 			
		...
		$test=$this->db->query("SELECT * FROM diskusi WHERE diskusi_id='$id'");
		$ddd=$test->row();

		if($this->input->post('returnDraft')=='true') {
			$status='draft';
			$add="";
			$published=($ddd->diskusi_was_published=="Y")?"Y":"N";
		} else {
			$status='publish';			
			$published="Y";
			$add="";
			if($ddd->diskusi_tgl_posting=='0000-00-00 00:00:00'){
					$add="diskusi_tgl_posting='$tanggal_posting',";
				} 
		} 
		...
		$update="UPDATE diskusi SET 
									...
									diskusi_sbg_headline='$headline',
									$add
									diskusi_tgl_last_edit='$tanggal_posting',
									diskusi_id_user_last_edit='$id_user_input',
									diskusi_editable='$editable',
									diskusi_seo_url='$seo',
									diskusi_meta_description='$meta_description',
									diskusi_meta_author='$meta_author',
									...
									diskusi_og_title='$og_title',
									diskusi_og_description='$og_description',
									diskusi_status='$status',
									diskusi_was_published='$published'
									WHERE
									diskusi_id='$id';
							";
				$this->db->query($update);
	...
	function delete_diskusi(){
		$id=changequote($this->input->post('id'));
		$diskusi_sql=$this->db->query("DELETE FROM diskusi WHERE diskusi_id='$id'");
		$foto_diskusi=$this->db->query("SELECT * FROM foto_diskusi WHERE id_diskusi='$id'");
		if($foto_diskusi->num_rows()>0){
			foreach ($foto_diskusi->result_array() as $row){
				//hapus file foto

				unlink(FCPATH."an-component/media/upload-gambar-diskusi/".$row['nama_foto']);

				unlink(FCPATH."an-component/media/upload-gambar-diskusi-thumbs/".$row['nama_foto']);

			}
			// hapus database foto
			$this->db->query("DELETE FROM foto_diskusi WHERE id_diskusi='$id'");
		}

		echo 'ok';
	}

	function delete_diskusi_foto(){
		$id=changequote($this->input->post('id'));
		$cari_foto=$this->db->query("SELECT * FROM foto_diskusi WHERE id_foto='$id'");
		if($cari_foto->num_rows()>0){
			$data=$cari_foto->row();
			//hapus file foto

				unlink(FCPATH."an-component/media/upload-gambar-diskusi/".$data->nama_foto);

				unlink(FCPATH."an-component/media/upload-gambar-diskusi-thumbs/".$data->nama_foto);

			//hapus  foto dari database
				$query=$this->db->query("DELETE FROM foto_diskusi WHERE id_foto='$id' ");
				if($query){
					echo "sukses";
				}
	...
	function delete_multi_photos_diskusi(){
		$id=explode(",",changequote($this->input->post("id")));
		foreach ($id as $value) {
			$cari=$this->db->query("SELECT * FROM foto_diskusi WHERE id_foto='$value'");
			if($cari->num_rows()>0){
				$data=$cari->row();
				...
				unlink(FCPATH."an-component/media/upload-gambar-diskusi-thumbs/".$data->nama_foto);
				$this->db->query("DELETE FROM foto_diskusi WHERE id_foto='$value'");
			}
		}
