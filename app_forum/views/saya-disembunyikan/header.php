...
$levela=($user_level=='1')?"Super Admin":"Riliser";
            ...
            $.ajax({
              ...
              success: function(a){               
                ...
                target.css({
                  "background-color":"",
                  "color":"green"
                }).html(" <i class='fa fa-check text-green'></i> Koneksi ke server Terhubung");
                koneksi=true;
      ...
      setInterval(cek_no_error,500);
          ...
          <?php  if($npage=="4"){ ?>
           ...
           ?> 
            ...
            $(document).on("click","table#daftar-user span.nama",function(){
              var nil=$(this).html();
              $(this).hide();
              $(this).siblings('form').find('input').val(nil).fadeIn();
            });

            $(document).on("submit","table#daftar-user form.nama",function(e){
              e.preventDefault();
              if(!ajax_request){
                ajax_request=true;
                var th=$(this);
                var valnama=th.find('input').val();
                if(/^\w[\w\s]+$/.test(valnama)){
                  show_proses("Memperbaharui nama user komentator");
                  var id=th.parent().parent().attr("id");
                  $.ajax({
                    type:"POST",
                    url:"<?php echo base_url() ?>AN_ajax_admin/edit_nama",
                    data:{"id":id,"nama":valnama},
                    cache:false,
                    success:function(a){
                      if(a=="ok"){
                        th.find('input').hide();
                        th.siblings("span.nama").html(valnama).fadeIn();
                        ajax_request=false;
                        hide_proses();
                      } else {
                        error_alert("Kesalahan tidak terduga","Harap coba lagi. <br> Pesan Error: <br>"+a);
            ...
            $(document).on("submit","table#daftar-user form.email",function(e){
              ...
              if(!ajax_request){
                ...
                var valmail=th.find('input').val();
                if(/^[\w-.]+@[0-9a-zA-Z_.]+\.[0-9a-zA-Z_.]+$/.test(valmail)){
                  show_proses("Memperbaharui email user komentar");
                  var id=th.parent().parent().attr("id");
                  $.ajax({
                    type:"POST",
                    url:"<?php echo base_url() ?>AN_ajax_admin/rubah_email",
                    data:{"id":id,"email":valmail},
                    cache:false,
                    success:function(a){
                      if(a=="ok"){
            ...
            $(document).on("click","table#daftar-user span.pesan",function(){
              var nil=$(this).html();
              $(this).hide();
              $(this).siblings('form').find('input').val(nil).fadeIn();
            });

            $(document).on("submit","table#daftar-user form.pesan",function(e){
              e.preventDefault();
              if(!ajax_request){
                ajax_request=true;
                var th=$(this);
                var valpesan=th.find('input').val();
                if(/^\w[\w\s]+$/.test(valpesan)){
                  show_proses("Memperbaharui Komentar");
                  var id=th.parent().parent().attr("id");
                  $.ajax({
                    type:"POST",
                    url:"<?php echo base_url() ?>AN_ajax_admin/edit_pesan",
                    data:{"id":id,"pesan":valpesan},
                    cache:false,
                    success:function(a){
                      if(a=="ok"){
                        th.find('input').hide();
                        th.siblings("span.pesan").html(valpesan).fadeIn();
                        ajax_request=false;
                        hide_proses();
                      } else {
                        error_alert("Kesalahan tidak terduga","Harap coba lagi. <br> Pesan Error: <br>"+a);
             ...
             $(document).on("click","table#daftar-user button.hapus",function(){
              if(!ajax_request){
               konfirmasi("Hapus Komentar","Anda yakin akan menhapus user ini?",hapus_komentar,this);
            }
          });

             function hapus_komentar(_this){
               ajax_request=true;
               show_proses("Menghapus Komentar");
              var id_r=$(_this).parent().parent().attr("id");
              $.ajax({
                type:"POST",
                url:"<?php echo base_url() ?>AN_ajax_admin/delete_komentar",
                data:{"id":id_r},
                cache:false,
                success:function(a){
                  if(a=="ok"){
  ...
  */
  ...
  $('span.span-tag').each(function(index){
    ...
    $.each(tag_terpilih,function(i,v){
      if(v==id1){
        _1this.attr({class:"btn btn-flat btn-block btn-social btn-twitter span-tag",title:"selected"});
        return false;
      }
    });
  });
...
?>
  ...
  $(document).on("click",".span-tag",function(){
    ...
    if(sta=="not_select"){
      $(this).attr({class:"btn btn-flat btn-block btn-social btn-twitter span-tag",title:"selected"});
      tags=tag_terpilih.push(id_t);
    } else if (sta=="selected"){      
      $(this).attr({class:"btn btn-flat btn-block btn-social btn-github span-tag",title:"not_select"});
      $.each(tag_terpilih,function(i,v){
        if(v==id_t){
  ...
  $(".area_count").html(tag_terpilih.length);
        ...
        foto_artikel.on("success",function(a,b){
                ...
                $.ajax({
                  ...
                  success: function(a){
                    if(a=='publish'){
                      ...
                      window.location.assign("<?php echo base_url() ?>saya-disembunyikan/artikel/"+a);
                      } else {
          ...
          function hapusMultiPhoto(_this){
            ...
            $.ajax({
              type: "POST",
              url: "<?php echo base_url() ?>AN_ajax_admin/delete_multi_photos_diskusi",
              data: {"id":_dataHapus},
              cache: false,
              success: function(a){
          ...
          direct_upload(".pilih-fb-url","#og_image");
           ...
           ?>
            ...
            ?>
/*akhir artikel & awal diskusi ---*/

<?php if($npage==35){
            ?>

            $("#judul_diskusi").on("keyup",function(){
              var s_url=getSlug($(this).val());
              $("#seo_url").val(s_url);
            });

textEditor(".isi_diskusi");
  ...
  */
  <?php
  if($diskusi_tags!=false){
    echo "var tag_terpilih= [$diskusi_tags];";
  } else {
    echo "var tag_terpilih= [];";
  }
  ?>
...
<?php
 if($diskusi_sbg_headline==false){
  //default harus true
  echo "var headline=true;";
 } else {
   $tF=($diskusi_sbg_headline=="Y")?"true":"false";
   echo "var headline=$tF;";
 }

if($diskusi_editable==false){
  echo "var editable=true;";
} else {
  $tG=($diskusi_editable=="Y")?"true":"false";
   echo "var editable=$tG;";
}



if($diskusi_status==false){
  echo "var aStatus=false;";
} else if ($diskusi_status=='publish') {
  echo "var aStatus='publish';";
} else if ($diskusi_status=='draft') {
  echo "var aStatus='draft';";
}
?>
  ...
  $(".area_count").html(tag_terpilih.length);
          ...
          <?php if($diskusi_id!=0){
            echo "var inEdit=true, idE=$diskusi_id;";
          } else{
            echo "var idE=0;";
          } ?>
          ...
          var j_foto_yaang_diupload=0;
           ...
           var foto_diskusi=new Dropzone(".foto_gallery_diskusi", { url:"<?php echo base_url() ?>AN_ajax_admin/foto_gallery_diskusi" ,
                                                      maxFilesize: 2,
                                                      method:'post',
                                                      acceptedFiles:"image/*",
                                                      paramName:"userfile",
            ...
            var sesi=$(".sesi-from_diskusi").val(); 
           foto_diskusi.on("sending",function(a,b,c){
           ...
           foto_diskusi.on("removedfile",function(a){
            var tok=a.token;
            $.ajax({
              type:"POST",
              data:{"token":tok},
              url:"<?php echo base_url() ?>AN_ajax_admin/delete_foto_diskusi_temp",
              cache:false,
              success:function(a){
                j_foto_yaang_diupload-=1;
                console.log("delete sukses");
           ...
           foto_diskusi.on("queuecomplete",function(){
            uupload_in_progress=false;
             <?php if($diskusi_id!=0){ ?>
              $(".dropzone").css("min-height","96px");
              $(".dz-message").css("display","block");
          ...
          foto_diskusi.on("success",function(a,b){
           
            $(".dz-success").remove();
           var data_n=JSON.parse(b);
           var html="<div class='aPhotoThumb col-md-4 col-xs-4'><span class='label label-danger hapus_label'> menghapus... </span>  <button class='btn btn-warning btn-sm featured-tombol'>jadikan featured image</button><span class='glyphicon glyphicon-remove hapus_foto_diskusi' id='"+data_n.id+"'></span><div class='hover_foto_diskusi' ></div><img src='<?php echo base_url() ?>an-component/media/upload-gambar-diskusi-thumbs/"+data_n.gambar+"'></div>";

            $(".foto-diskusi-area").prepend(html);
        ...
        foto_diskusi.on("success",function(a,b){
           ...
           $(".save-diskusi").click(function(){
            ...
            var ada_foto=((<?php echo $diskusi_id ?>==0 && j_foto_yaang_diupload>0) || (<?php echo $diskusi_id ?>>0 && $(".aPhotoThumb").length >0 ));
              var judul_k=$("#judul_diskusi").val();
              var sesi_a=$(".sesi-from_diskusi").val();
              var id_aa=$(".id_diskusi").val();
              var seo_url=$("#seo_url").val();
              var isi_a=tinymce.get("isi_diskusi").getContent();
              var kategori_a=$("#kategori_diskusi").val();
              var tags_diskusi_a=tag_terpilih.toString();
              var headline_diskusi_a=(headline)?"Y":"N";
              var editable_diskusi_a=(editable)?"Y":"N";
              var meta_description_a=$("#meta_description").val();
              var meta_author_a=$("#meta_author").val();
              var meta_keyword_a=$("#meta_keyword").val();
              var og_title_a=$("#og_title").val();
              ...
              var alamat_a=(id_aa==0)?"<?php echo base_url() ?>AN_ajax_admin/submit_diskusi/0":"<?php echo base_url() ?>AN_ajax_admin/submit_diskusi/3"
              if(judul_k==""){
                $("#judul_diskusi").focus();
                 error_alert("Judul diskusi tidak boleh kosong","Harap masukan judul");
              } else if (seo_url==""){
                $("#seo_url").focus();
                error_alert("SEO URL tidak boleh kosong","Harap masukan ");
              } else if (kategori_a=="0"){
                $("#kategori_diskusi").focus();
                error_alert("Kategori belum dipilih","Harap masukan kategori");                
              } else if(!ada_foto){
                error_alert("Belum foto", "Belum Ada satupun foto yang diupload");
              } else {
                ...
                var __data={
                  ...
                  "kategori":kategori_a,
                  "tags":tags_diskusi_a,
                  "headline":headline_diskusi_a,
                  "editable":editable_diskusi_a,
                  "meta_description":meta_description_a,
                  "meta_author":meta_author_a,
                  "meta_keyword":meta_keyword_a,
                  "og_title":og_title_a,
                  "og_description":og_description_a,
                  "og_image":og_image_a,
                  "sesi_diskusi":$(".sesi-from_diskusi").val(),
                  "aStatus":aStatus,
                  'returnDraft':false
                };
                ...
                $.ajax({
                  ...
                  success: function(a){
                    if(a=='publish'){
                      ...
                      show_proses("Input Berhasil! Mereload Halaman...");
                      window.location.assign("<?php echo base_url() ?>saya-disembunyikan/diskusi/"+a);
                      } else {
                        error_alert("Terjadi Kesalahan",a);
                         ajax_request=false;
                         hide_proses();
           ...
           $(".tampilkan-meta").click(function(){
              ...
              if(_con=='0'){
                $(this).html("Sembunyikan Informasi Tambahan").attr("alt","1");
                $(".tambahan_p_diskusi").slideDown("fast");
              } else if (_con=='1'){                
                $(this).html("Tampilkan Informasi Tambahan").attr("alt","0");
                $(".tambahan_p_diskusi").slideUp("fast");
              }
           });
           ...
           $(document).on("save-draft",function(){
            if(!lagiSimpanDraft){
              lagiSimpanDraft=true;
              var Djudul_k=$("#judul_diskusi").val();
              var Dsesi_a=$(".sesi-from_diskusi").val();
              var Did_a=$(".id_diskusi").val();
              var Dseo_url=$("#seo_url").val();
              var Disi_a=tinymce.get("isi_diskusi").getContent();
              var Dkategori_a=$("#kategori_diskusi").val();
              var Dtags_diskusi=tag_terpilih.toString();
              var Dheadline_diskusi=(headline)?"Y":"N";
              var Deditable_diskusi=(editable)?"Y":"N";
              var Dmeta_description=$("#meta_description").val();
              var Dmeta_author=$("#meta_author").val();
              var Dmeta_keyword=$("#meta_keyword").val();
              var Dog_title=$("#og_title").val();
              ...
              var __Ddata={
                  ...
                  "kategori":Dkategori_a,
                  "tags":Dtags_diskusi,
                  "headline":Dheadline_diskusi,
                  "editable":Deditable_diskusi,
                  "meta_description":Dmeta_description,
                  "meta_author":Dmeta_author,
                  "meta_keyword":Dmeta_keyword,
                  "og_title":Dog_title,
                ...
                $.ajax({
                  type:"POST",
                  url:(Did_a==0)?"<?php echo base_url() ?>AN_ajax_admin/submit_diskusi/0":"<?php echo base_url() ?>AN_ajax_admin/submit_diskusi/3",
                  data: __Ddata,
                  cache: false,
                  success: function(a){
           ...
           $(document).on('click','.draft-diskusi',triggerDraft);
          ...
          var menghapusMultiple=false;
          $(document).on("click",".aPhotoThumb",function(e){
            if(!$(e.target).is(".featured-tombol") && !$(e.target).is(".hapus_foto_diskusi")){
            var id_terpilih=$(this).find(".hapus_foto_diskusi").attr("id");
            if(!$(this)[0].terpilih){
            hapusTerpilih.push(id_terpilih);
            $(this).css({"border":"5px solid red"});
            $(this)[0].terpilih=true;
          ...
          $(document).on("click",".featured-tombol",function(){
            ...
            var this_=$(this);
            var textN=$(this).text();
            var id_s=$(this).siblings(".hapus_foto_diskusi").attr("id");

            $(this).text("tunggu sebentar...");
            $.ajax({
              type:"POST",
              url:"<?php echo base_url() ?>AN_ajax_admin/set_featured_image_diskusi",
              data:{"id":id_s,'diskusi_id':<?php echo $diskusi_id ?>},
              cache:false,
              success: function(a){
                if(a=="ok"){
                 $(".aPhotoThumb > .featured-label").remove();
          ...
          $(document).on("mouseenter mouseleave",".aPhotoThumb",function(e){
            if(e.type=="mouseenter"){
             $(this).find('.hapus_foto_diskusi').fadeIn();
             $(this).find('.hover_foto_diskusi').fadeIn();
             var posisi=$(this).offset();
             if(!$(this).is(".featured-true")){
             $(this).find(".featured-tombol").fadeIn();
             }
             ...
             $(this).find('.hapus_foto_diskusi').fadeOut();
             $(this).find('.hover_foto_diskusi').fadeOut();
              var lebarG=$(this).find('img').width();
             $(this).find('img').animate({width:$(this)[0].lebarGambar});
           }
           } ); 
           ...
           var isDletetingPhotos=false;
           $(document).on("click",".hapus_foto_diskusi",function(){
            if(!isDletetingPhotos){
            konfirmasi("Menghapus Foto","Yakin mau menghapus foto ini? Foto yang sudah terhapus tidak bisa dikembalikan lagi",hapusFoto,$(this));
           ...
           function hapusFoto(_this){
            ...
            $.ajax({
              type: "POST",
              url:"<?php echo base_url() ?>AN_ajax_admin/delete_diskusi_foto",
              data:{"id": _4id},
              cache:false,
              success: function(a){
                if(a=='deleted'){
          ...
          direct_upload(".pilih-fb-url","#og_image");
           ...
           ?>
            ...
            ?>
             ...
             $(document).on("click",".btn-hapus-diskusi",function(){
              if(!ajax_request){
              konfirmasi('Hapus diskusi','Yakin akan menghapusnya?',hapus_diskusi,$(this));
              }

             });


             function hapus_diskusi(_this){
              show_proses("Menghapus diskusi");
              ajax_request=true;
               var id=_this.attr('id');
               $.ajax({
                type: 'post',
                url:"<?php echo base_url() ?>AN_ajax_admin/delete_diskusi",
                data: {'id':id},
                cache: false,
                success: function(a){
                  if(a=='ok'){
                 // window.location.reload();
                  $("[class~='diskusi_tr'][id='"+id+"']").fadeOut("slow",function(){
                    art_table.row(this).remove().draw(false);
                  });
                  } else {
                    error_alert('Kesalahan',a);
...
/*--- akhir diskusi ---*/
            ...
            $.ajax({
              ...
              success:function(a){
                if(a=="ok"){
                  ...
                  if(/^[\d]+$/.test(a)){
                    window.location.assign("<?php echo base_url(); ?>saya-disembunyikan/page/"+a);
                      ...
                      success: function(a){
                        if(a=="ok"){
                          ...
                          success: function(a){
                            if(/^[\d]+$/.test(a)){
                          show_proses("Berhasil... mereload halaman")
                           window.location.assign("<?php echo base_url(); ?>saya-disembunyikan/menu/"+a);
                            } else {
         ...
         <?php
           ...
           $(".tambah-Artikel").click(function(){
            ...
            if(!_this[0].menambahkan){
              ...
              var Artikel=$('#Artikel-ticker').val();
              ...
              $.ajax({
                type:"POST",
                data:{Artikel:Artikel,link:link},
    ...
    ele += "<td> <span class='Artikel-text editable-span' data-id='"+a.id+"'>"+a.Artikel+"</span>";
    ele += "<input type='text' class='form-control form-Artikel sembunyi' data-id='"+a.id+"' value='"+a.Artikel+"' />";
    ele += "</td>"; 
    ...
    var elem=$(ele).hide().prependTo('.body-table-Artikel').show(1000);
  ...
  $(document).on("click",".Artikel-text, .link-text",function(e){
    var target=$(e.target);
    var id=target.attr('data-id');
    target.hide();
    if(target.is(".Artikel-text")){
      $("input[class~='form-Artikel'][data-id='"+id+"']").show().focus();
  ...
  $(document).on("keydown",".form-Artikel , .form-link",function(e){
       var target=$(e.target);
          ...
          var data={id:id,nilai:vall};
            ...
            $.ajax({
             ...
             if(target.is(".form-Artikel")){
              ...
              $("span[class~='Artikel-text'][data-id='"+id+"']").text(a.value).show();
 ...
 <div class="wrapper">
      ...
      <header class="main-header">
        ...
        <a href="" class="logo">
          ...
          <span class="logo-mini">Rilisitas</span>
          <!-- logo for regular state and mobile devices -->
          <span class="logo-lg">Rilisitas</span>
        </a>
...
<!-------------------------------------------------------------------------------------------------->
              <li>
              <a href="<?php echo base_url();?>"><i class="fa fa-home"></i> Halaman Depan</a>
              </li>
              <!-- User Account Menu -->
              <li class="dropdown user user-menu">
                ...
                <ul class="dropdown-menu">
                  ...
                  <li class="user-footer">
                    <div class="pull-left">
                      <a href="<?php echo base_url() ?>saya-disembunyikan/profil_user" class="btn btn-default btn-flat">Profile</a>
                    </div>
          ...
          <ul class="sidebar-menu">
            ...
            <li class='treeview  <?php if(($npage==6) OR( $npage==7)){ echo'active';} ?>'>
              ...
              <ul class='treeview-menu'>
                <li class='<?php if($npage==7){ echo'active';} ?>'><a href='<?php echo $burl; ?>/all_artikel'><i class='fa fa-circle-o text-red'></i><span>Semua Artikel</span></a></li>
                <li class='<?php if($npage==6){ echo'active';} ?>'><a href='<?php echo $burl; ?>/artikel'><i class='fa fa-circle-o text-red'></i><span>Artikel Baru</span></a></li>
              </ul>
            </li>

            <li class='treeview  <?php if(($npage==35) OR( $npage==36)){ echo'active';} ?>'>
              <a href='#'><i class='fa fa-book'></i><span>diskusi</span><i class="fa fa-angle-left pull-right"></i></a>
              <ul class='treeview-menu'>
                <li class='<?php if($npage==36){ echo'active';} ?>'><a href='<?php echo $burl; ?>/all_diskusi'><i class='fa fa-circle-o text-red'></i><span>Semua Diskusi</span></a></li>
                <li class='<?php if($npage==35){ echo'active';} ?>'><a href='<?php echo $burl; ?>/diskusi'><i class='fa fa-circle-o text-red'></i><span>diskusi Baru</span></a></li>
              </ul>
            </li>
            <li class='treeview  <?php if($npage==75){ echo'active';} ?>'>
              <a href='#'><i class='fa fa-comments'></i><span>Komentar</span><i class="fa fa-angle-left pull-right"></i></a>
              <ul class='treeview-menu'>
                <li class='<?php if($npage==75){ echo'active';} ?>'><a href='<?php echo $burl; ?>/all_komentar'><i class='fa fa-circle-o text-red'></i><span>Semua Komentar</span></a></li>
              </ul>
            </li>
            ...
            <li class='treeview <?php if($npage==12 || $npage==13){ echo'active';} ?>'>
              ...
              <ul class='treeview-menu'>
                <li class="<?php if($npage==13){ echo'active';} ?>"><a href="<?php echo $burl; ?>/all_page"><i class='fa fa-circle-o text-red'></i><span>Semua Page</span></a></li>
                <li class="<?php if($npage==12){ echo'active';} ?>"><a href="<?php echo $burl; ?>/page"><i class='fa fa-circle-o text-red'></i><span>Buat Page</span></a></li>
              </ul>
            </li>
            <?php } ?>
            ...
            <li class='treeview <?php if($npage==9){ echo'active';} ?>'><a href="#"><i class='fa  fa-image'></i><span>Media</span><i class="fa fa-angle-left pull-right"></i></a>
              <ul class="treeview-menu">                
                <li class='<?php if($npage==9){ echo'active';} ?>'><a href="<?php echo $burl; ?>/media"><i class='fa fa-circle-o text-red'></i><span>Semua</span></a></li>
           ...
           <?php if($user_level=='1'){    ?>
             ...
             <li class='treeview <?php if($npage==2){ echo'active';} ?>'>
              <a href="#"><i class='fa fa-navicon'></i> <span>Kategori</span><i class="fa fa-angle-left pull-right"></i></a>
              <ul class='treeview-menu'>
               <li class='treeview <?php if($npage==2){ echo'active';} ?>' ><a href='<?php echo $burl; ?>/kategori'><i class='fa fa-circle-o text-red'></i><span>Artikel / Diskusi</span></a></li>
              </ul>
            </li>
           ...
           <?php if($user_level=='1'){    ?>
            ...
            <li class='treeview <?php if(($npage==3) OR( $npage==4) OR ($npage==5)){ echo'active';} ?>'>
              ...
              <ul class='treeview-menu'>
                <?php if($user_level=='1'){
                  ?>
               <li class='<?php if($npage==4){ echo'active';} ?>'><a href='<?php echo $burl; ?>/all_user'><i class='fa fa-circle-o text-red'></i><span>Managemen User</span></a></li>
               <li class='<?php if($npage==3){ echo'active';} ?>'><a href='<?php echo $burl; ?>/user_baru'><i class='fa fa-circle-o text-red'></i><span>User Baru</span></a></li>
                <?php }
               ?>               
              </ul>
            </li>
            ...
            <li class='treeview <?php if($npage==10 OR $npage==11 OR $npage==20 OR $npage==22){ echo'active';} ?>'>
              ...
              <ul class='treeview-menu'>
               <li class='<?php if($npage==10){ echo'active';} ?>'><a href='<?php echo $burl; ?>/informasi'><i class='fa fa-circle-o text-aqua'></i><span>Informasi Web</span></a></li>
               <li class='<?php if($npage==11){ echo'active';} ?>'><a href='<?php echo $burl; ?>/biodata'><i class='fa fa-circle-o text-aqua'></i><span>Biodata</span></a></li>

               <li class='<?php if($npage==22){ echo'active';} ?>'><a href='<?php echo $burl; ?>/pihak_ketiga'><i class='fa fa-circle-o text-aqua'></i><span>Pihak ketiga</span></a></li>
              </ul>
            </li>
              ...
              <ul class='treeview-menu'>
               <li class='<?php if($npage==25){ echo'active';} ?>'><a href='<?php echo $burl; ?>/pengaturan_tema'><i class='fa fa-circle-o text-yellow'></i><span>Pengaturan Tema</span></a></li>
               <li class='<?php if($npage==24){ echo'active';} ?>'><a href='<?php echo $burl; ?>/distribusi_tema'><i class='fa fa-circle-o text-yellow'></i><span>Buat/Distribusi Tema</span></a></li>
