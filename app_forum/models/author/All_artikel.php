...
class All_artikel extends CI_Model {
	...
	function get_artikel(){
		$cari=$this->db->query("SELECT * FROM artikel WHERE artikel_terhapus='N' AND artikel_id_user = '".$this->session->userdata("id_user")."' ORDER BY artikel_id DESC");
		...
		if($cari->num_rows()>0){
			...
			foreach ($cari->result_array() as $value) {
				if($this->session->userdata('id_user')==$value['artikel_id_user'] || $this->session->userdata('id_user')==1 || $value['artikel_editable']=='Y'){
					...
					$edit_button="<a href='".base_url()."author/artikel/$value[artikel_id]'><i class='fa fa-edit btn-edit-artikel'></i></a>";
				}else{
