...
class Pages {
	...
	function detail_page($id){
		$data=$this->CI->db->query("SELECT page_id AS id,
		 page_judul AS judul,
		  page_url AS slug,
		   page_isi AS isi,
		    page_foto AS foto,
		     page_javascript AS javascript,
		     page_meta_keywords AS meta_keywords,
		      page_meta_description AS meta_description FROM pages WHERE page_id='$id' AND page_status='published' ");
